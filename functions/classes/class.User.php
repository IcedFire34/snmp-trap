...
private function fetch_user_details($username) {
    // Sadece nesne aktif değilse
    if (!is_object($this->user)) {
        try {
            $user = $this->Database->findObject("users", "username", $username);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), true);
            return; // Hata durumunda metodu sonlandırın
        }

        // Kullanıcı nesnesini diziye dönüştür
        $usert = (array) $user;
        ...
        if (empty($usert)) {
            ...
            if (isset($user->role)) {
                ...
                $this->Result->show("danger", _("User role is not defined"), true);
            }
        }

        // İzin verilen hostları ve panel düzenini ayarla
